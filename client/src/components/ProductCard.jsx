...
export default function ProductCard({ product }) {
  ...
  return (
    ...
    >
      {/* Image area - white background like Blinkit */}
      <div style={{ position:'relative', background:'#f8f8f8', height:160, display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', padding:12 }}>
        <img
          ...
          onError={e => {
            ...
            e.target.parentNode.innerHTML += `<div style="font-size:52px;text-align:center;opacity:0.4">📦</div>`
          }}
        />

        {/* Discount badge */}
        {disc > 0 && (
          <div style={{ position:'absolute', top:8, left:8, background:'#0c831f', color:'#fff', fontSize:10, fontWeight:800, padding:'3px 8px', borderRadius:6 }}>
            {disc}% off
          </div>
        )}
        ...
        {/* Product badge */}
        {product.badge && (
          <div style={{ position:'absolute', top:8, right:8, background: badgeColor[product.badge]||'#888', color:'#fff', fontSize:10, fontWeight:800, padding:'3px 8px', borderRadius:6 }}>
      ...
      <div style={{ padding:'10px 12px 14px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          {product.stock === 0 ? null : inCart ? (
            <div style={{ display:'flex', alignItems:'center', background:'#f0fff4', borderRadius:8, overflow:'hidden', border:'1.5px solid #0c831f' }}>
